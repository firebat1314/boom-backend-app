...
@Component({
  selector: 'sss-operate',
  templateUrl: './operate.page.html',
  styleUrls: ['./operate.page.scss'],
})
export class OperatePage implements OnInit {
  ...
  getDataList(data = {}, options?: HttpOptions) {
    return this.apiServ.operateList(Object.assign(this.formData, data), options).pipe(
      map((res) => {
        if (res && res.code === 0) {
          this.data = res;
